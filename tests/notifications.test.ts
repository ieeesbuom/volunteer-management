...
describe("notifications", () => {
  ...
  it("keeps SMTP email disabled unless explicitly enabled by environment", async () => {
    vi.resetModules();
    vi.doMock("@/lib/env", () => ({
      getServerEnv: () => ({
    ...
    const { createNotificationEmailAdapter } = await import(
      "../src/features/notifications/server/email-adapter"
    );
    const adapter = createNotificationEmailAdapter();
    const delivery = await adapter.sendNotification({
      subject: "Disabled",
      text: "No SMTP send should be attempted.",
    ...
    expect(delivery).toMatchObject({
      disabled: true,
      provider: "disabled",
      reason: expect.stringContaining("NOTIFICATION_EMAILS_ENABLED=true"),
      safeRecipient: "pe***@example.com",
    });
  });
